...
/// inside the user's session: a handler that throws or blocks makes the agent
/// feel slow, and a memory system may never be the reason someone's editor
/// stalls.
fn opencode_plugin(exe: &Path) -> String {
    format!(
        r#"// rolepod-brain capture for OpenCode 2. Generated by `brain setup`.
//
...
// instance per project. Every handler here is fire-and-forget and swallows
// its own errors: capture must never slow down or break the session it is
// observing.
//
  ...
  async setup(ctx) {{
    ...
    const onEvent = (ev) => {{
      ...
      if (ev?.type === "session.created") {{
        ...
        if (agent) return
        capture("SessionStart", {{ cwd: cwdOf(session_id), session_id, source: "startup" }})
        return
      }}
