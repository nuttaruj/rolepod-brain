...
pub fn targets_in(home: &Path, exe: &Path) -> Result<Vec<Target>> {
    ...
    Ok(vec![
        ...
        Target {
            ...
            timeout_overrides: &[],
            matchers: &[],
            // Verified against the OpenCode 2 event schema (`@opencode/schema`
            // 2.0.12) and a probe plugin on the live service. The end of a turn
            // is the session boundary; OpenCode has no session-end event, and
            // its `session.idle` never fires in headless runs. OpenCode 1's
            // `session.compacted` is now `session.compaction.ended`.
            events: &[
                "session.created",
...
///
/// Written to the OpenCode 2 contract (`@opencode/plugin` 2.x): a default
/// export with an `id` and a `setup(ctx)`. OpenCode 1 loaded a named factory
/// export instead; OpenCode 2 refuses that shape outright ("Plugin must export
/// a default definition"), which is how a machine that updated on 2026-09-21
/// captured nothing from OpenCode for a day. `brain doctor` now tells the two
/// apart, see [`plugin_source_is_current`].
///
/// Since OpenCode 2 a plugin runs inside the shared background service, one
/// instance per project: `process.cwd()` is the home directory and the
/// environment is the service's. The project path comes from `ctx.location`
/// and from the session events themselves, never from the process.
///
/// Everything here is wrapped, and only two handlers wait: a session's first
/// request, for its primer, and a completed tool call, for what memory holds
/// about its file - each bounded by `ANSWER_WAIT_MS`. A plugin runs inside
/// the user's session: a handler that throws or blocks makes the agent feel
/// slow, and a memory system may never be the reason someone's editor stalls.
fn opencode_plugin(exe: &Path) -> String {
    format!(
        r#"// rolepod-brain for OpenCode 2. Generated by `brain setup`.
//
// OpenCode has no hook configuration file; plugins subscribe in code, and
// since OpenCode 2 they run inside the shared background service, one
// instance per project. Every handler swallows its own errors, and capture
// is fire-and-forget: memory must never break the session it is observing.
// Only memory handed back to the model is waited for, and never for long.
//
// Dependency-free on purpose: `Plugin.define` from `@opencode/plugin` returns
// its argument unchanged, so the plain object below is the same plugin
...
// How long a hook whose answer we read may hold up a turn. The capture
// finishes either way; only the answer is dropped.
const ANSWER_WAIT_MS = 2000
// brain must have its answer built by then, less this, or build none: its
// last write and the pipe back need the time.
const ANSWER_MARGIN_MS = 500

// The same hook, but its answer is read: what memory holds for this moment,
// as text to put in front of the model, or null. `reads_answer` tells the
// hook someone is listening - without it, brain builds nothing for OpenCode,
// because an injection nobody reads still spends the session's budget - and
// `answer_by` tells it until when.
function ask(event, payload) {{
  const answer_by = Date.now() + ANSWER_WAIT_MS - ANSWER_MARGIN_MS
  return new Promise((resolve) => {{
    let out = ""
    const timer = setTimeout(() => resolve(null), ANSWER_WAIT_MS)
    const done = (text) => {{
      clearTimeout(timer)
      resolve(text)
    }}
    try {{
      ...
      child.on("close", () => {{
        try {{
          done(JSON.parse(out)?.hookSpecificOutput?.additionalContext || null)
        }} catch {{
          done(null)
        }}
      }})
      child.stdin.end(JSON.stringify({{ ...payload, reads_answer: true, answer_by }}))
      child.unref()
    }} catch {{
      done(null)
    }}
...
export default {{
  ...
  async setup(ctx) {{
    // The service's cwd is $HOME and its env is not the shell's: the project
    // is whatever this instance was created for, and nothing else.
    const instanceDir = ctx.location?.directory ?? ctx.location?.project?.directory ?? null
    // No project, no capture: a memory filed under a guess is worse than a
    // missing one, and without a directory the guess would be the home.
    if (!instanceDir) return

    // The event stream is the whole service's, every project at once, and
    // only `session.created` says which project a session belongs to. So:
    // remember the sessions created here, and ignore every later event for
    // a session this instance did not see start - a session opened before
    // the plugin loaded loses its boundaries, never its tool calls, because
    // the tool hook below is scoped to this instance already.
    //
    // A child session (`parentID` set) is a subagent: its start and its end
    // are not the lead's boundaries, and its tool calls are tagged as its
    // own work rather than filed as the lead's.
    const sessions = new Map()
    const cwdOf = (id) => sessions.get(id)?.cwd ?? instanceDir
    // The primer each lead session was handed, as a pending answer.
    const primers = new Map()

    const onEvent = (ev) => {{
      const data = ev?.data ?? {{}}
      const session_id = data.sessionID ?? null
      if (ev?.type === "session.created") {{
        // Ours if it opened in this directory, or is a child of a session
        // that did: a subagent's session is filed where its lead's is.
        const ours = data.location?.directory === instanceDir || sessions.has(data.parentID)
        if (!ours) return
        const agent = data.parentID ? String(data.agent ?? "subagent") : null
        sessions.set(session_id, {{ cwd: data.location?.directory ?? instanceDir, agent }})
        if (agent) return
        primers.set(session_id, ask("SessionStart", {{ cwd: cwdOf(session_id), session_id, source: "startup" }}))
        return
      }}
      if (ev.type === "session.deleted") {{
        sessions.delete(session_id)
        primers.delete(session_id)
        return
      }}
      const known = sessions.get(session_id)
      if (!known || known.agent) return
      switch (ev.type) {{
        // The end of a turn. `session.idle` is the documented name for it,
        // but it is a status event that headless runs never emit; the
        // execution event is the engine's own and fires in every mode.
        case "session.execution.succeeded":
          capture("Stop", {{ cwd: known.cwd, session_id }})
          return
        case "session.compaction.ended":
          capture("PreCompact", {{ cwd: known.cwd, session_id, trigger: data.reason ?? "auto" }})
          return
      }}
    }}
    ...
    void (async () => {{
      try {{
        ...
        // is the one failure worth a line in OpenCode's log, because from
        // here on session boundaries are silently lost.
        if (!stop.signal.aborted) console.error("rolepod-brain: event stream ended:", error)
      }}
    }})()

    // Measured on the live service: unlike the event stream, the tool hook
    // is scoped to this instance, so a session created before the plugin
    // loaded still captures its calls, filed under the instance directory.
    const onTool = async (e) => {{
      try {{
        const session_id = e?.sessionID ?? null
        const payload = {{
          cwd: cwdOf(session_id),
          session_id,
          tool_name: String(e?.tool ?? ""),
          tool_input: e?.input ?? {{}},
        }}
        const agent = sessions.get(session_id)?.agent
        if (agent) {{
          // The keys Claude Code uses for the same fact, so the hook applies
          // the same rule: a delegate's footsteps are not the lead's.
          payload.agent_id = session_id
          payload.agent_type = agent
        }}
        // Only a completed call has a result to carry memory back in; a
        // delegate is never handed any. Both are captured and nothing more.
        if (agent || e?.status !== "completed" || !e?.result) {{
          capture("PostToolUse", payload)
          return
        }}
        // What memory holds about the file just touched rides back inside
        // the tool's own result, the next thing the model reads.
        const text = await ask("PostToolUse", payload)
        if (text) e.result.content = [...(e.result.content ?? []), {{ type: "text", text }}]
      }} catch {{
        /* never break a tool call */
      }}
    }}
    try {{
      await ctx.tool.hook("execute.after", onTool)
    }} catch (error) {{
      console.error("rolepod-brain: tool hook not registered:", error)
    }}

    // OpenCode rebuilds the system prompt for every request, so the primer
    // goes back into each one: asked for once, paid for once from the
    // budget, and still there after a compaction. Title and summary calls
    // are the session's own bookkeeping and get none.
    const onContext = async (e) => {{
      try {{
        if (e?.kind && e.kind !== "primary") return
        const text = await primers.get(e?.sessionID)
    ...
    try {{
      ...
      console.error("rolepod-brain: context hook not registered:", error)
    }}
...
/// Is the plugin file on disk the shape the installed OpenCode will load?
///
/// OpenCode 2 needs a default export; the file OpenCode 1 loaded has none and
/// fails at startup with a warning only the log sees. A present file is not
/// an installed plugin, and the doctor should say which one it found.
#[must_use]
pub fn plugin_source_is_current(path: &Path) -> bool {
    std::fs::read_to_string(path).is_ok_and(|text| plugin_text_is_current(&text))
}

fn plugin_text_is_current(text: &str) -> bool {
    text.contains("export default") && text.contains(&format!("id: {PLUGIN_NAME:?}"))
}

/// Does the plugin on disk hand memory back, or only capture?
///
/// One written before it read any answer is the right shape and loads, so it
/// passes [`plugin_source_is_current`] - but it never says `reads_answer`,
/// and an OpenCode session gets no memory pushed until `setup` rewrites it.
#[must_use]
pub fn plugin_source_delivers(path: &Path) -> bool {
    std::fs::read_to_string(path).is_ok_and(|text| text.contains("reads_answer: true"))
}
...
mod tests {
    ...
    /// the shape the installed OpenCode reads, subscribe to every event the
    /// target claims, and take nothing from the process it runs in.
    #[test]
    fn the_opencode_plugin_is_the_opencode_2_shape() {
        let exe = Path::new("/usr/local/bin/brain");
        let source = opencode_plugin(exe);

        assert!(source.contains("export default {"));
        assert!(source.contains("id: \"rolepod-brain\""));
        assert!(source.contains("async setup(ctx)"));
        assert!(source.contains("ctx.event.subscribe({ signal:"));
        assert!(source.contains("ctx.tool.hook(\"execute.after\""));
        assert!(source.contains("const BRAIN = \"/usr/local/bin/brain\""));

        // Memory goes back to the model: the primer into every request's
        // system prompt, file pointers into the tool's result - and both only
        // through `ask`, whose stdout is read and whose payload says so.
        assert!(source.contains("ctx.session.hook(\"context\""));
        assert!(source.contains("e.system.push({ type: \"text\", text })"));
        assert!(source.contains("e.result.content = [...(e.result.content ?? []), { type: \"text\", text }]"));
        assert!(source.contains("stdio: [\"pipe\", \"pipe\", \"ignore\"]"));
        assert!(source.contains("reads_answer: true, answer_by"));
        assert!(source.contains("primers.set(session_id, ask(\"SessionStart\""));
        assert!(source.contains("await ask(\"PostToolUse\""));

        // The service's cwd is the home directory, never a project, and a
        // dependency would be a package the user has to install.
        assert!(!source.contains("process.cwd()"));
        assert!(!source.contains("from \"@opencode"));
        // The OpenCode 1 shape, gone for good: v2 ignores a named factory.
        assert!(!source.contains("export const"));
        assert!(!source.contains("tool.execute.after\":"));

        // Every event the target lists is one the plugin actually handles.
        let target = targets(exe)
            .unwrap()
            .into_iter()
            .find(|target| target.kind.as_str() == "opencode")
            .expect("an opencode target");
        for event in target.events {
            let handled = match *event {
                // The tool hook is registered by its short name.
            ...
            assert!(
                dispatched.iter().any(|needle| source.contains(needle)),
                "plugin never dispatches on {event}"
            );
        }
    }

    /// The doctor has to tell an OpenCode 1 file from a current one: both are
    /// "present", and only one of them will ever run.
    #[test]
    fn a_plugin_file_of_the_old_shape_is_not_current() {
        assert!(plugin_text_is_current(&opencode_plugin(Path::new("/x/brain"))));
        let v1 = "import { spawn } from \"node:child_process\"\n\
                  export const RolepodBrainPlugin = async ({ directory }) => ({})\n";
        assert!(!plugin_text_is_current(v1));
        assert!(!plugin_text_is_current(""));

        let dir = std::env::temp_dir().join(format!("brain-plugin-{}", ulid::Ulid::new()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("rolepod-brain.js");
        assert!(!plugin_source_is_current(&file), "a missing file is not current");
        std::fs::write(&file, v1).unwrap();
        assert!(!plugin_source_is_current(&file));
        std::fs::write(&file, opencode_plugin(Path::new("/x/brain"))).unwrap();
        assert!(plugin_source_is_current(&file));
        assert!(plugin_source_delivers(&file));

        // The first OpenCode 2 plugin: loads fine, hands nothing back.
        let captures_only = "export default {\n  id: \"rolepod-brain\",\n  async setup(ctx) {},\n}\n";
        std::fs::write(&file, captures_only).unwrap();
        assert!(plugin_source_is_current(&file));
        assert!(!plugin_source_delivers(&file));
